...
uses Windows{$IFDEF USEDBGOUT}, SysUtils{$ENDIF};

type
  HMemModule = NativeUInt;

function MemLoadLibrary(const buf: Pointer): HMemModule; {$IFDEF USESTDCALL}stdcall; {$ENDIF}
function MemGetProcAddress(Module: HMemModule; const lpProcName: LPCSTR): FARPROC; {$IFDEF USESTDCALL}stdcall; {$ENDIF}
function MemFreeLibrary(Module: HMemModule): BOOL; {$IFDEF USESTDCALL}stdcall; {$ENDIF}

implementation

type
  TFNDllMain  = function(hinstDLL: HMODULE; Reason: Integer; Reserved: Pointer): BOOL; stdcall;
...
procedure DoCopySections(const data: PAnsiChar; old_headers: PImageNtHeaders; var Module: TMemModule);
var
  I               : Integer;
  dest            : Pointer;
...
begin
  ...
  section := PImageSectionHeader(PAnsiChar(@Module.Headers^.OptionalHeader) + Module.Headers^.FileHeader.SizeOfOptionalHeader);
...
begin
  Result := False;
  dir    := PImageDataDirectory(@Module.Headers.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT]);
  // 没有任何导入函数,不存在导入表
  ...
  begin
    ...
    dbgout(string(PAnsiChar(@Module.CodeBase[importDesc^.Name])));
    if hm = 0 then
      Exit;
    ...
    begin
      thunk := PIMAGE_THUNK_DATA(@Module.CodeBase[importDesc^.OriginalFirstThunk]);
    end
    else
    begin
    ...
    begin
      ...
      begin // 按序号
        func^ := Windows.GetProcAddress(hm, PAnsiChar(thunk^.Ordinal and $0000FFFF));
      end
      else // 按函数名
      begin
        pByName := PIMAGE_IMPORT_BY_NAME(@Module.CodeBase[thunk^.AddressOfData]);
...
begin
  ...
  dir := PImageDataDirectory(@Module.Headers.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC]);
  if (dir.VirtualAddress = 0) or (dir.Size = 0) then
  begin
  ...
  begin
    ...
    for I := 0 to (relocation.SizeOfBlock - SizeOf(IMAGE_BASE_RELOCATION)) div SizeOf(Word) - 1 do
    begin
...
{$IFDEF WIN64}
        IMAGE_REL_BASED_DIR64:
          begin
            ...
            dbgout('IMAGE_REL_BASED_DIR64(=%x).', [PUINT64(UINT64(BlockAddr) + offset)^]);
          end;
{$ENDIF}
      else
        begin
...
var
  ...
  pgsize                  : NativeInt;
begin
  imageOffset := {$IFDEF WIN64} Module.Headers.OptionalHeader.ImageBase and $FFFFFFFF00000000; {$ELSE}0{$ENDIF};
  section     := PImageSectionHeader(PAnsiChar(Module.Headers) + SizeOf(TImageNtHeaders));

  for I := 0 to Module.Headers.FileHeader.NumberOfSections - 1 do
  begin
    protectvalue := 0;
    if (section^.Characteristics and IMAGE_SCN_MEM_EXECUTE) = IMAGE_SCN_MEM_EXECUTE then
      protectvalue := protectvalue or PAGE_EXECUTE;
    if (section^.Characteristics and IMAGE_SCN_MEM_READ) = IMAGE_SCN_MEM_READ then
      protectvalue := protectvalue or PAGE_READONLY;
    if (section^.Characteristics and IMAGE_SCN_MEM_WRITE) = IMAGE_SCN_MEM_WRITE then
      protectvalue := protectvalue or PAGE_WRITECOPY;
    if (section^.Characteristics and IMAGE_SCN_MEM_NOT_CACHED) = IMAGE_SCN_MEM_NOT_CACHED then
      protectvalue := protectvalue or PAGE_NOCACHE;

    if (section^.Characteristics and IMAGE_SCN_MEM_DISCARDABLE) = IMAGE_SCN_MEM_DISCARDABLE then
    begin
      VirtualFree(Pointer(section^.Misc.PhysicalAddress or imageOffset), section^.SizeOfRawData, MEM_DECOMMIT);
      Continue;
    end;
    ...
    if pgsize = 0 then
    begin
      if (section^.Characteristics and IMAGE_SCN_CNT_INITIALIZED_DATA) = IMAGE_SCN_CNT_INITIALIZED_DATA then
      begin
        pgsize := Module.Headers.OptionalHeader.SizeOfInitializedData;
      end
      else if (section^.Characteristics and IMAGE_SCN_CNT_UNINITIALIZED_DATA) = IMAGE_SCN_CNT_UNINITIALIZED_DATA then
      begin
    ...
    if pgsize > 0 then
    begin
      VirtualProtect(Pointer(section^.Misc.PhysicalAddress or imageOffset), pgsize, protectvalue, oldProtect);
    end;
...
var
  dir                : PImageDataDirectory;
  EnrtyNums          : DWORD;
  RtlAddFunctionTable: function(FunctionTable: PImageRuntimeFunctionEntry; EntryCount: DWORD; BaseAddress: DWORD64): boolean; stdcall;
begin
  @RtlAddFunctionTable := GetProcAddress(GetModuleHandle(kernel32), 'RtlAddFunctionTable');
  if not Assigned(RtlAddFunctionTable) then
    Exit;

  dir := PImageDataDirectory(@Module.Headers.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION]);
  if (dir.VirtualAddress = 0) or (dir.Size = 0) then
  begin
    dbgout('No SEH found.');
    Exit;
  ...
  if not RtlAddFunctionTable(Module.RuntimeFunc, EnrtyNums, DWORD64(Module.CodeBase)) then
...
begin
  ...
  // 尝试在ImageBase的位置分配内存, 如果分配失败就由系统指派
  ret^.CodeBase := PAnsiChar(VirtualAlloc(Pointer(pOld^.OptionalHeader.ImageBase), pOld^.OptionalHeader.SizeOfImage, MEM_COMMIT or MEM_RESERVE, PAGE_EXECUTE_READWRITE));
  if ret^.CodeBase = nil then
    ret^.CodeBase := PAnsiChar(VirtualAlloc(nil, pOld^.OptionalHeader.SizeOfImage, MEM_RESERVE, PAGE_EXECUTE_READWRITE));
  if ret^.CodeBase = nil then
  begin
    Dispose(ret);
  ...
  // 真正按code地址分配
  VirtualAlloc(ret^.CodeBase, pOld^.OptionalHeader.SizeOfImage, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  VirtualAlloc(ret^.CodeBase, pOld^.OptionalHeader.SizeOfHeaders, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
  ret^.Headers := PImageNtHeaders(@ret^.CodeBase[pDos^._lfanew]);
  CopyMemory(ret^.CodeBase, pDos, pOld^.OptionalHeader.SizeOfHeaders);
  ret^.Headers                          := PImageNtHeaders(@ret^.CodeBase[pDos^._lfanew]);
  ret^.Headers.OptionalHeader.ImageBase := TNativeAddr(ret^.CodeBase); // 修正ImageBase如果原有ImageBase被占用,结果分配到不同地方
  DoCopySections(_buf, pOld, ret^);                                    // 拷贝Section
  // 查看是否需要重定位.
  ImageBaseDelta := Cardinal(ret^.CodeBase) - pOld^.OptionalHeader.ImageBase;
...
{$ENDIF}
  ...
  if ret^.Headers.OptionalHeader.AddressOfEntryPoint <> 0 then // 设置和调用入口函数
  begin
    ret^.EntryPoint := TFNDllMain(@ret^.CodeBase[ret^.Headers.OptionalHeader.AddressOfEntryPoint]);
    if (Assigned(ret^.EntryPoint)) then
    begin
      try
        if (not ret^.EntryPoint(HMODULE(ret^.CodeBase), DLL_PROCESS_ATTACH, nil)) then
        begin
          MemFreeLibrary(HMemModule(@ret));
          Exit;
        end;
...
{ GetProcAddress }
function MemGetProcAddress(Module: HMemModule; const lpProcName: LPCSTR): FARPROC;
type
...
begin
  ...
  dir := PImageDataDirectory(@pm^.Headers.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT]);
  ...
  if (idx < 0) or (idx > Integer(exps^.NumberOfFunctions)) then
    Exit;
...
var
  pm                    : LPMemModule;
  I                     : Integer;
  RtlDeleteFunctionTable: function(FunctionTable: PImageRuntimeFunctionEntry): boolean; stdcall;
begin
  Result := False;
  pm     := LPMemModule(Module);
  if (pm = nil) or (pm^.Magic <> MEMMODULEMAGICNUM) then
  ...
  begin
    @RtlDeleteFunctionTable := GetProcAddress(GetModuleHandle(kernel32), 'RtlDeleteFunctionTable');
